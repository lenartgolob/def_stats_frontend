  ...
  function Trie() {
    ...
    function insert(word) {
      ...
      for (let i = 0; i < word.length; i++) {
        const current_letter = word[i].toLowerCase();
    ...
    function find(clue_letters) {
      ...
      for (let i = 0; i < clue_letters.length; i++) {
        const clue_letter = clue_letters[i].toLowerCase();
    ...
    function findAllWords(node, arr) {
      if (node.isComplete) { // check if node is end node
        arr.unshift(node.originalWord); // get original word and add it to array
      }
